...
def ask_gemini(prompt_text):
    ...
    try:
        ...
        if "error" in res_data:
            return f"❌ Gemini 에러: {res_data['error'].get('message', 'Unknown')}"

        candidates = res_data.get("candidates", [])
        if not candidates:
            return "❌ Gemini 응답이 비어 있어요."

        parts = candidates[0].get("content", {}).get("parts", [])
        text = "".join(part.get("text", "") for part in parts if part.get("text"))
        return text or "❌ Gemini 텍스트 응답이 비어 있어요."
...
Please in Korean"""
    ...
    return normalize_summary_text(ask_gemini(prompt))
